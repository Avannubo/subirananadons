                    ...
                    <div className="flex flex-col md:flex-row md:justify-between md:items-end items-center justify-center text-center gap-2 md:gap-0">
                        ...
                        <div className="flex flex-col items-center md:items-start w-full md:w-auto">
                            ...
                            <div className="flex justify-center items-center gap-[12px] mt-4 md:mt-2">
                                ...
                                {socials && socials.map(({ key, link, Icon, name }) => (
                                    <Link key={key} href={link} aria-label={name} className="text-[#333] hover:text-[#36A9E1] transition-colors cursor-pointer" target="_blank" rel="noopener noreferrer">
                                        <Icon className="h-[25px] w-[25px]" />
                                    </Link>
                                ))}
                                {/* WhatsApp icon */}
                                {whatsapp && (
                                    <Link href={`https://wa.me/${whatsapp}`} aria-label="WhatsApp" className="text-[#333] hover:text-[#36A9E1] transition-colors cursor-pointer" target="_blank" rel="noopener noreferrer">
                                        <svg className="h-[34px] w-[34px] text-gray-700 hover:text-[#36A9E1]" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                                ...
                                {/* Teléfono icon */}
                                {telefono && (
                                    <a href={`tel:${telefono}`} aria-label="Teléfono" className="text-[#333] hover:text-[#36A9E1] transition-colors cursor-pointer">
                                        <Phone className="h-[25px] w-[25px]" />
                                    </a>
                                )}
                                {/* Email icon */}
                                {email && (
                                    <Link href={`mailto:${email}`} aria-label="Email" className="text-[#333] ml-1 mt-[1px] hover:text-[#36A9E1] transition-colors cursor-pointer">
                        ...
                        <div className="flex flex-row justify-center md:justify-end items-center w-full md:w-auto mt-6 md:mt-0">
                            <ul className="space-y-2 md:space-y-2 text-center md:text-left w-full">
                                <li><Link href="/terms" className="hover:text-[#36A9E1] font-medium transition-colors cursor-pointer">{t('terms')}</Link></li>
                                <li><Link href="/privacy" className="hover:text-[#36A9E1] font-medium transition-colors cursor-pointer">{t('privacy')}</Link></li>
                                <li><Link href="/cookies" className="hover:text-[#36A9E1] font-medium transition-colors cursor-pointer">{t('cookies')}</Link></li>
                                <li><Link href="/legal" className="hover:text-[#36A9E1] font-medium transition-colors cursor-pointer">{t('legal')}</Link></li>
                            </ul>
                        </div>
